...
def execProgram(instrs, initRegs):
    ...
    output = []
    goodOutput = True
    while (pc in range(len(instrs))) and (len(output) < 100) and goodOutput: 
        instr = instrs[pc]
        if instr[0] == "cpy":
            ...
            if type(x) is int:
                ...
                valX = int(x[1:len(x)])
            output.append(valX)
            evenLenO = len(output) - (len(output) % 2)
            good = [0,1] * (evenLenO // 2)
            if output[0:evenLenO] != good:
                goodOutput = False
            pc += 1
        else:
...
ia = 0
while True:
    go = execProgram(loadInstrs(src), [ia,0,0,0])
    if go:
        break
    ia += 1
print "Part one: " + str(ia)
print "(No part two for this day)"
