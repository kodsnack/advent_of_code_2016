...
*/
DEFINE VARIABLE iLastSolution AS INTEGER     NO-UNDO.
...
REPEAT:
    ...
    iSolution = 0.
    FOR EACH ttpath NO-LOCK WHERE ttPath.step < 51 BREAK BY ttPath.px BY ttPath.py:
        IF FIRST-OF(ttPath.py) THEN DO:
            iSolution = iSolution + 1.
        END.
    END.

    /* Same as last count? */
    IF iSolution = iLastSolution THEN
        LEAVE pathfinding.
    
    /* Save last result */
    iLastSolution = iSolution.
