...
export function generateInstructions(input:string){
	return input.replace(/\s+/, '').split(",").map(i => new Instruction(i));
}

export var Instructions = generateInstructions(input);
