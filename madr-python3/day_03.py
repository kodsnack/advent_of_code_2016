...
def find_fake_triangles_ver(puzzle):
    ...
    return sum([map(lambda t: t[0] + t[1] > t[2],
                ...
                [(list(map(int, re.findall(r'^\s+(\d+)', puzzle, flags=re.MULTILINE))) + list(
                    map(int, re.findall(r'^\s+\d+\s+(\d+)', puzzle, flags=re.MULTILINE))) + list(
                    map(int, re.findall(r'^\s+\d+\s+\d+\s+(\d+)', puzzle, flags=re.MULTILINE))))]][0])
