...
parseInstr = choice [try parseCpy, try parseInc, try parseDec, try parseJnz, try parseTgl, try parseMul, try parseNop] <* optional newline
  where
    parseCpy = Cpy <$>
    ...
    parseNop = string "nop" >> return Nop
...
evalOne (ins, m) (Mul d r) = (ins, withReg succ (setRegValue m d (t0 * t1)) 'p')
  where
    t0 = getRegValue m d
    t1 = getRegValue m r
evalOne (ins, m) Nop = (ins, withReg succ m 'p')
...
evalOne (ins, m) (Tgl (Right v)) =
  ...
  in flip (maybe (ins, withReg succ m 'p')) i $ \ i' ->
    ...
    (Mul _ _) -> error "trying to run 'tgl' on 'mul' is undefined"
    Nop -> error "trying to run 'tgl' on 'nop' is undefined"
...
optimize ins = S.take 2 ins S.>< multIns S.>< S.drop 10 ins
  where
    multIns = S.fromList [Mul 'a' 'b', Nop, Nop, Nop, Nop, Nop, Nop, Nop]

main :: IO ()
main = do
  program <- (head <$> getArgs) >>= readProgram
  let (_, a, _, _, _) = eval (optimize $ S.fromList program) startMachine
  print a
