...
whatChipInOutput rncs n = rnc >>= getOutput n
  where
    rnc = find (\ ((Rule _ low high), _) -> ((Left n) == low) || ((Left n) == high)) rncs

    getOutput n ((Rule _ low high), cs)
      | low == (Left n) = Just $ minimum cs
      | high == (Left n) = Just $ maximum cs
      | otherwise = Nothing

main :: IO ()
main = do
  (state, rules) <- (head <$> getArgs) >>= readInstructions
  let rncs = unfoldr (step rules) state
      prod = product <$> mapM (whatChipInOutput rncs) [0, 1, 2]
  putStrLn $ maybe "Failed to find product" show $ prod
